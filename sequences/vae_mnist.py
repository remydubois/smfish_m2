...
clean = numpy.load(LOCAL_SOURCE + 'sequences_clean.npy')
train_set, test_set = train_test_split(
    list(zip(*clean))[0], train_size=0.8, test_size=0.2)
x_train = next(batch_generator_vae(
        train_set, batch_size=-1, repeat=5))[0]
x_test = next(batch_generator_vae(
        test_set, batch_size=-1, repeat=5))[0]
